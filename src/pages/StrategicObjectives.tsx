...
export function StrategicObjectives() {
    ...
    const [epicData, setEpicData] = useState<Record<string, { progress: number, hours: number, summary?: string, status?: string, children?: any[] }>>({})
    ...
    const fetchEpicProgress = async (currentObjectives: StrategicObjective[], forceRefresh = false) => {
        ...
        try {
            ...
            const progressMap: Record<string, { progress: number, hours: number, summary?: string, status?: string, children?: any[] }> = {}
            epics.forEach((e: any) => {
                progressMap[e.key] = {
    ...
    const executeSendStrategicEmail = async (targetEmails: string, ccEmails: string = "") => {
        ...
        try {
            ...
            const totalEffort = objectives.reduce((acc, obj) => acc + obj.epicKeys.reduce((eAcc, key) => eAcc + (epicData[key]?.hours || 0), 0), 0).toFixed(0);
                ...
                <div style="max-width: 800px; margin: 0 auto; background-color: #ffffff; border-radius: 40px; overflow: hidden; box-shadow: 0 40px 100px -20px rgba(0, 21, 64, 0.15); border: 1px solid #f1f5f9;">
                    ...
                    <div style="padding: 0 40px 40px;">
                        <div style="border: 1px solid #f1f5f9; border-top: none; border-radius: 0 0 20px 20px; overflow: hidden;">
                            ${objectives.map(obj => {
                const actualProg = getObjectiveProgress(obj)
                const isManual = obj.suggestedProgress != null
                const hours = obj.epicKeys.reduce((acc, key) => acc + (epicData[key]?.hours || 0), 0)

                // Get tickets for this objective
                const tickets = obj.epicKeys.flatMap(key => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-[#FF4200] p-8 rounded-[32px] text-white flex flex-col justify-between shadow-xl shadow-orange-200/50 h-36 relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-start relative z-10">
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/60">Percentual Apurado</p>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 rounded-[32px] flex flex-col justify-between shadow-xl shadow-slate-200/40 h-36">
                    <div className="flex justify-between items-start">
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Total Investido (Horas)</p>
                        <BarChart3 size={20} className="text-slate-300" />
                    </div>
                    <h2 className="text-5xl font-black leading-none text-[#001540] dark:text-white">
                        {objectives.reduce((acc, obj) => acc + obj.epicKeys.reduce((eAcc, key) => eAcc + (epicData[key]?.hours || 0), 0), 0).toFixed(1)}h
                    </h2>
                </div>
            </div>
